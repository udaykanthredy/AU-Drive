...
import { X, Copy, Check, Globe, Link2, Loader2, Clock, Folder as FolderIcon } from 'lucide-react';
...
export function ShareModal() {
  const { shareFileId, setShareFile, shareFolderId, setShareFolder } = useUIStore();
  const [copied, setCopied] = useState(false);
  const [expiresInDays, setExpiresInDays] = useState<number | null>(7);
  const [shareLink, setShareLink] = useState<string | null>(null);

  // Derive resource info from whichever is set
  const isOpen = !!(shareFileId || shareFolderId);
  const resourceId = shareFileId ?? shareFolderId ?? null;
  const resourceType: 'file' | 'folder' = shareFileId ? 'file' : 'folder';

  // Only fetch file metadata when sharing a file
  const { data: fileWrapper, isLoading: fileLoading } = useQuery({
  ...
  const { mutate: generateLink, isPending } = useMutation({
    mutationFn: () => sharesApi.createShare(resourceId!, resourceType, expiresInDays),
    onSuccess: (data) => {
  ...
  const isMetaLoading = shareFileId ? fileLoading : false;
  const title = resourceType === 'folder' ? 'Share Folder' : 'Share File';
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 border border-gray-800 rounded-xl shadow-2xl w-full max-w-md z-50 p-6 animate-in zoom-in-95">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-950 border border-gray-800 rounded-lg p-3 flex items-center gap-3">
              {resourceType === 'folder' ? (
                // Folder — no async fetch needed, just show a folder icon
                <>
                  <FolderIcon className="w-8 h-8 flex-shrink-0 text-brand-400" />
                  <div className="overflow-hidden">
                    <p className="text-sm font-medium text-gray-200 truncate">Selected Folder</p>
                    <p className="text-xs text-gray-500">Viewers can browse and download files</p>
                  </div>
                </>
              ) : isMetaLoading || !fileWrapper ? (
