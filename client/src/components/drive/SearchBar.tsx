  ...
  return (
    <div ref={wrapperRef} className="relative flex-1 max-w-2xl mr-6">
      <div className="relative group">
        ...
        <input
          type="text"
          className="block w-full pl-10 pr-10 py-2.5 border border-gray-700 rounded-xl leading-5 bg-gray-900/50 text-gray-100 placeholder-gray-400 focus:outline-none focus:bg-gray-900 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all sm:text-sm"
      ...
      {isOpen && (query || results.length > 0) && (
        <div className="absolute z-50 mt-2 w-full bg-gray-900 border border-gray-800 rounded-xl shadow-2xl overflow-hidden backdrop-blur-xl">
          {results.length > 0 ? (
            <ul className="max-h-96 overflow-y-auto py-2">
              {results.map((result) => (
                <li
                  key={result.file_id}
                  className="px-4 py-3 hover:bg-gray-800 cursor-pointer border-b border-gray-800/50 last:border-0 transition-colors"
                ...
                >
                  ...
                  <p className="text-xs text-gray-400 line-clamp-2 pl-6 italic">
                    "...{result.snippet}..."
                  </p>
                </li>
              ))}
